...
- 🎥 **实时视觉检测**：基于 YOLOv8 / ONNXRuntime 的实时物体识别
- 💬 **自然语言理解**：基于 Qwen-VL，支持中英文口语化指令
- 🧠 **智能动作规划**：自动生成机械臂执行序列
- 🤖 **3D 物理仿真**：MuJoCo UR5e 桌面场景验证
- 🎨 **现代化界面**：Gradio 实时交互界面
...
第一代的重点是验证“视觉检测 → 指令理解 → 动作规划 → 机械臂执行”这条完整链路。

### 第二代：Qwen-VL 与实时仿真升级

第二代在第一代基础上进行了模型和工程结构升级：

- 将语言视觉模型更换为 **Qwen-VL**，用于解析桌面操作指令
- 对 Qwen-VL 进行量化更新，降低显存占用，提高本地运行可行性
- 将实时摄像头读取和 YOLO 检测改为后台线程，减少 UI 阻塞
- 支持 ONNXRuntime 实时检测后端，并保留 PyTorch / TensorRT 扩展能力
- 统一内部规划标签为英文，例如 `杯子 → cup`，提高目标匹配稳定性
- 接入 MuJoCo UR5e 仿真 viewer
- 增加桌子、杯子和机械臂的桌面仿真场景
- 增加检测结果、动作计划、执行状态和仿真窗口展示

第二代的目标是从基础功能验证升级到实时运行、模型优化和可观察的机械臂仿真执行。

## 📸 第二代效果预览

### MuJoCo 桌面机械臂仿真

![MuJoCo 机械臂桌面场景](images/mujoco_demo.png)

### Qwen-VL 指令理解与执行

![Qwen-VL 指令执行结果](images/demo.png)

### 实时检测与动作规划

![实时检测与动作规划](images/demo2.png)
